...
구현됨:
- 공통 응답 포맷(`ApiResponse`), 전역 예외 처리(`GlobalExceptionHandler`, `ErrorCode`)
- JPA Auditing 베이스 엔티티, Redis 템플릿 설정, OpenAPI 설정
- `performance` 도메인 수직 슬라이스: 엔티티 · 리포지토리 · 서비스 · 컨트롤러 · Flyway `V1__init.sql`
- 프론트: 라우팅, QueryClient, axios 클라이언트, 공연 목록/상세 페이지
